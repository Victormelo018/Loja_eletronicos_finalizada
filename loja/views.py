...
def adicionar_ao_carrinho(request, produto_id):
    if not request.user.is_authenticated:
        messages_api.error(request, 'Você precisa estar logado para adicionar produtos no carrinho.')
        return redirect('login')
    
    produto = get_object_or_404(Produto, id=produto_id)

    if not produto.disponivel or produto.quantidade <= 0:
        messages_api.error(request, f'0 produto "{produto.nome}" não está disponivel.')
